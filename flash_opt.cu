...
// The tensor cores (for matmul) on Voltas are different compared to the tensor cores on Turing and Ampere,
// and the shared memory layout required is different. Each Volta tensor core multiplies 4 matrices of shape 8x4 and 4x4
// (i.e. the mma.sync.aligned.m8n8k4 instruction).
// Each Turing and Ampere tensor core multiply 1 matrix of shape 16x8 and 8x8, or 16x16 and 16x8
// (i.e. the mma.sync.aligned.m16n8k8 and mma.sync.aligned.m16n8k16 instruction).
// This means that inputs to the mma instructions need to be laid out differently in shared memory.